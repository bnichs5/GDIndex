...
(function () {
  ...
  async function onGet(request) {
	...
	if (path.endsWith('.strm')) {
      return new Response('https://gdindex-demo.suck-me-wang-mate.workers.dev' + encodeURI(path.slice(0, -5)), {
        status: 200,
        headers: {
    ...
    if (path.startsWith('/~_~_gdindex/resources/')) {
      ...
      const r = await fetch(`https://raw.githubusercontent.com/bnichs5/Index-GD/master/web/dist/${remain}`);
